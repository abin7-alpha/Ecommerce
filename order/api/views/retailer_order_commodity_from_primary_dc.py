...
from order.functions import sendOrderConfirmationMailToCustomer
from order.functions import add_order_items_to_order, upload_invoice_to_aws, send_order_sms, order_async_functions
...
def get_order_no(order):
	...
	datetime_start = datetime.strptime(sales_after, "%d/%m/%Y")
	datetime_end = datetime.strptime(sales_before, "%d/%m/%Y")+timedelta(hours=23,minutes=59,seconds=59)
	allsalesThisFy = Order.objects.filter(created__gte=datetime_start, created__lte=datetime_end).all()
	print(allsalesThisFy)
...
def confirm_retailer_order_commodities_from_primary_dc(request):
	...
	try:
		...
		if retailer_obj.is_payment_check_required == True and not retailer_obj.pending_amount_limit:
			...
			primary_dc = DistributionCenter.objects.get(is_primary_dC=True)

			order.amount = grand_total
			order.dispatch_dc = primary_dc
			order.save()
		...
		#retailer who have pending amount limit
			...
			sum_of_order_and_oustanding = current_order_amount + current_outstanding_amount

			if sum_of_order_and_oustanding > retailer_obj.pending_amount_limit:
				output["status"] = "order_failed"
				output["status_text"] = "Your credit exceeded, please contact admin!"
		...
		#For retailer who verified by the admin who dont have to pay.
			...
			if is_partial_order_successful:
				output['status']="partial_order_success"
